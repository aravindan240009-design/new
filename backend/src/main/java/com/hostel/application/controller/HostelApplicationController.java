...
public class HostelApplicationController {
    ...
    public ApiResponse<HostelApplicationResponse> create(@Valid @RequestBody HostelApplicationRequest request) {
        return ApiResponse.ok("Application submitted successfully", service.create(request));
    }

    @GetMapping
    public ApiResponse<List<HostelApplicationResponse>> all() {
        return ApiResponse.ok("Applications fetched", service.findAll());
    }

    @GetMapping("/{id}")
    public ApiResponse<HostelApplicationResponse> one(@PathVariable Long id) {
        return ApiResponse.ok("Application fetched", service.findById(id));
    }
    ...
    public ApiResponse<HostelApplicationResponse> update(@PathVariable Long id, @Valid @RequestBody HostelApplicationRequest request) {
        return ApiResponse.ok("Application updated", service.update(id, request));
    }
    ...
    public ApiResponse<HostelApplicationResponse> approve(@PathVariable Long id) {
        return ApiResponse.ok("Application approved", service.approve(id));
    }

    @PatchMapping("/{id}/reject")
    public ApiResponse<HostelApplicationResponse> reject(@PathVariable Long id) {
        return ApiResponse.ok("Application rejected", service.reject(id));
    }

    @PatchMapping("/{id}/room")
    public ApiResponse<HostelApplicationResponse> room(@PathVariable Long id, @Valid @RequestBody RoomAllotmentRequest request) {
        return ApiResponse.ok("Room allotted", service.allotRoom(id, request));
    }
    ...
    @GetMapping("/filter")
    public ApiResponse<List<HostelApplicationResponse>> filter(@RequestParam(required = false) ApplicationStatus status,
                                                               @RequestParam(required = false) Gender gender,
                                                               @RequestParam(required = false) String course) {
        return ApiResponse.ok("Filter completed", service.filter(status, gender, course));
    }
